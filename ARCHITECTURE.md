...
- `"likelihood"` (default): leave `"Z"` in the spec so the **engine** estimates λ jointly with the structural parameters by maximum likelihood (its joint-λ BFGS + anchor-snap over `{−2,−1,−0.5,0,0.5,1,2}`), bounded below by `lambdaLower`.  No R-side screen.
