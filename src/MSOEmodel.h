...
struct BSMmodel{
    ...
    bool MSOE = true,          // MSOE model or BSM
         PTSnames = true;      // PTS model names or UC
    // OUTPUTS:
...
void BSMclass::setModel(string model, vec periods, vec rhos, bool runFromConstructor){
    ...
    if (trend != "?" && cycle != "?" && seasonal != "?" && irregular != "?"){  // One model
        ...
        if (inputs.MSOE)
            bsm2msoe();
        bool errorExit __attribute__((unused)) = checkSystem(SSmodel::inputs.system);
    }
...
void BSMclass::initMatricesBsm(vec periods, vec rhos, string trend, string cycle, string seasonal, string irregular){
    ...
    // SSmodel::inputs.system.ns = SSmodel::inputs.system.T.n_cols;
    // SSmodel::inputs.system.ns = SSmodel::inputs.y.n_rows;
    // bool errorExit = checkSystem(SSmodel::inputs.system);
}
// Initializing parameters of BSM model
